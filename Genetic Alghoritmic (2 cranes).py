...
def convert(chromo):
    ...
    orderly = []
    for i in range(1,len(chromo)+1):
        orderly.append(chromo.index(i))
    return orderly


class Genetic:

    def __init__(self,teta,Teta,n,N,bNumb,vicinity,C1,C2,m):
    ...
    def run(self,population = None):
        ...
        if population == None:
            ...
            time = [self.time(bNumb,vicinity,chromo,Pre,C1,C2) for chromo in first]
        ...
        while round(best[0],8)> 70:
            cont+=1
            self.population = self.new_population(self.n,self.teta,self.Teta,self.population,Pre,Pos,C1,C2,self.m)
            best = min(self.population, key = lambda x:x[0])
            print cont,best[0]
    ...
    def time(self,bNumb,vicinity,chromo,Pre,C1,C2):
        ...
        times = [t_vec(x,y,bNumb) for x,y in zip(chromo,vic)]
        M1,M2,time = self.decodification(chromo,times,Pre,bNumb,C1,C2)

        return time

    def decodification(self,chromo,times,Pre,bNumb,C1,C2):

        m1=[]       #ja começa com o job 1
        m2=[]

        time_m1 = 0 # ja começo com o tempo do job 1 em todas
        time_m2 = 0

        built={}

        for B,T in zip(chromo,times):

            pre = Pre[B]
            time=0

            weight = bNumb[B].w
            if C1 > weight and C2> weight:
                start_time = 0
                for pre_b in pre:     # devo buscar em que tempo o block pode começar a ser edificado
                    builts = built.keys()
                    if pre_b in built:
                        t = built[pre_b]
                        if t>start_time:
                ...
                list_sorted = [[time,'prior. = 4','t'],[time_m1,'prior. 3',m1],[time_m2,'prior. 2',m2]]
                list_sorted.sort()

                for i in range(len(list_sorted)):
                    if list_sorted[i][2]=='t':
                        index = i

                if index== 1:
                    pre_time = list_sorted[0][0]
                    ...
                    # IF INDEX == 2 => CHOSE "index-1" THAT IS "1" ; IF INDEX == 0 => CHOSE "index+1" THAT IS "1"
                    maq = list_sorted[1][2]
                ...
                if maq == m1:
                    ...
                    time = time + T
                    time_m1 = time
                    #print 'job = ',job,'  Time = ',time_m1, '  Machine 1'
                    ...
                    time = time + T
                    time_m2 = time
                    #print 'job = ',job,'  Time = ',time_m2, '  Machine 2'

                built[B]=time

            elif C1 > weight and C2< weight:
    ...
    def new_population(self,n,teta,Teta,population,Pre,Pos,C1,C2,m):
        ...
        while len(new_population) != size:
            ...
            if (ch1 == p1).all():
                ...
                t1 = self.time(bNumb,vicinity,ch1,Pre,C1,C2)
            ...
            if (ch2 == p1).all():
                ...
                t2 = self.time(bNumb,vicinity,ch2,Pre,C1,C2)
...
if __name__ == "__main__":
    ...
    G = Genetic(0.35,.8,4,300,bNumb,vicinity,350,350,MoP)
...
#    order = csv_read('Ordens/Grande bloco_Popa-Proa.csv')
#    order = csv_read('Ordens/Camada_Popa-Proa.csv')
