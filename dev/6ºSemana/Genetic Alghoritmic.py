...
class Genetic:
    ...
    def run(self,population = None):
        ...
        if population == None:
            first = [self.gen_chromosome(Pos,self.m) for i in range(self.N)]
    ...
    def gen_chromosome(self,Pos,m):
        ...
        while itens!=[]:
            ...
            if gene :chromo = np_app(chromo,gene)
            itens.remove(gene)
            
            for j in Pos[gene]:
                s = 0
                for i in set(np_app(chromo,gene)):
                    s += m[i][j]
                    if s==1:
                        itens.append(j)
                        break
    ...
    def time(self,bNumb,vicinity,chromo):
        ...
        time = sum((t_vec(x,y) for x,y in zip(chromo,vic)))
